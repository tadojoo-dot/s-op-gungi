...
### 차이금액 — 개별 행은 부호, 합계는 절대합 (2026-08-19 사용자 지정)

**총계·합계 자리의 차이금액은 전부 `Σ|차이|`다.** 순합(실적합−계획합)은 화면에서 뺐다 —
바로 옆 오차율(WAPE)이 절대값 기준이라 "차이 0억인데 오차율 40%"가 나와 헷갈렸기 때문.
지금은 **오차율 × 계획 = 차이금액**이 정확히 성립한다(26/8 7월: 35.25% × 76.75 = 27.06억).

| 자리 | 값 |
|---|---|
| 보정 대상 리스트 **개별 행** `차이금액` | `actual−plan` (부호 유지, 초과 초록/미달 빨강) |
| 보정 대상 리스트 **총계 행** (`renderHT`) | `totals.over+Math.abs(totals.under)` |
| ③탭 KPI 「계획 대비 차이금액」 (`renderHitKpis`) | `tOver+Math.abs(tUnder)` · 색 중립(terra) |
| 채널×월 팝업 헤더 `차이합계` + 표 `합계` 행 (`renderChDrill`) | `absSum` |

- ⚠ **순합(bias)으로 되돌리지 말 것.** 방향(초과/미달)은 `계획 초과 금액`·`계획 미달 금액` KPI 카드와
  총계 오차율 밑 `초과 X% + 미달 Y%` 보조문구가 담당한다.
- ⚠ `renderChDrill`에서 `net` 변수는 **삭제했다**. 개별 행 `r.diff`는 부호 그대로다(정렬·색이 그걸 쓴다).
...
### 적중률 = 1 − 오차율 (2026-08-19 사용자 지정)

"오차율이 100%를 넘는 게 문제다 · 적중률로 보고 싶다"는 요청. **오차율의 정확한 뒤집기**를 쓴다 —
`hitRateFromWape(w) = Math.max(0, 100-w)`. 그래서 **적중률 + 오차율 = 100%**가 성립한다.

- ⚠ **0에서 자른다.** 오차율은 분모가 계획뿐이라 위쪽이 안 막혀 100%를 넘을 수 있다.
  26/8 파일 70칸 중 **12칸**이 그렇고, 표에 보이는 6·7월에서는 **3칸**(제로샵 6·7월, 오프라인기타 6월,
  최대 −7%)이다. 심한 건 전부 4월(카카오 −736% / 제로샵 −261% / 벤더 −198% / 쿠팡 −78%)인데 4월은 표시 밖이다.
- ⚠ **100% 초과는 계산 사고가 아니다.** 두 경로로 생긴다:
  - ① 실적이 계획을 크게 초과 (26/8 4월 카카오: 계획 0.91억 → 실적 6.80억 = 836%)
  - ② **품목별 절대오차가 상쇄 없이 쌓임** — 채널 총액은 **미달인데도** 100%를 넘는다.
    26/8 7월 제로샵: 계획 0.538 · 실적 0.216인데 품목 |차이| 합 **0.575** → 107%.
    계획한 품목은 안 팔리고 계획에 거의 없던 품목이 팔린 경우 = 구성까지 틀린 상태
- ⚠ **분모가 '계획'인 한 100% 초과는 구조적으로 못 막는다.** 검토했다가 기각한 안들:
  - `실적/계획`(달성률) — **여전히 100%를 넘고**(26/8 7월 쿠팡 132%, 4월 카카오 745%),
    상쇄 때문에 문제 채널이 멀쩡해 보인다(7월 직영몰 97%인데 실제 오차 10.26억).
    게다가 이미 ③탭 `renderChAchieve`(`actual/plan*100`)가 그 값이다
  - `Σmin/Σmax`, sMAPE 계열 — 0~100% 보장되지만 오차율과 값이 어긋나 설명 비용이 생긴다
- 잘린 칸은 `isHitClamped(w)` → `.cw-hit0`(회색) + title로 실제 오차율을 알린다.
  **값 자체는 바로 옆 오차율 열에 그대로 있다 — 열을 나란히 둔 이유다.**
- 정렬 키는 `hr:<월키>` 프리픽스로 오차율 열과 가른다(`chWapeSortValue`).
  ⚠ 적중률 정렬 = 오차율 정렬의 역순이 **아니다** — 잘린 칸이 0%로 동률이 되기 때문.

### 채널별 오차율 표 — 표시 규칙 (2026-08-18, 폭·타이포는 2026-08-19 개편)

| | |
|---|---|
| 적중률 열 | 오차율 그룹 **바로 오른쪽**에 같은 월 수·**같은 폭(`W_MO`)·같은 타이포**로 붙인다(2026-08-19 사용자 지정: "동일한 열로"). 위계를 주지 않는다. 그룹 배경도 같은 톤이고 경계는 `cw-sep` 세로선뿐. ⚠ 두 열은 합이 100이라 **4개 열이 실질 정보 2개**다 — 그래도 나란히 두는 게 사용자 결정이다. 표 `min-width`는 1184 → **1368px**(패널 ~1500px 안) |
...
### 채널×월 오차 상세 팝업 (`openChDrill`/`renderChDrill`) — 2026-08-19 개편

- 표에 `.chwape-drill` 모디파이어가 붙는다. **모든 열 가운데 정렬 · 여백 없이 넓게**(사용자 지정).
  ⚠ 규칙은 전부 `.chwape-drill` 안에 가둘 것 — `.chwape td.l{text-align:left}`와 첫 열 sticky는
  채널별 오차율 표(`.chwape-lg`)가 쓰는 **공용** 규칙이라 base에서 건드리면 그쪽이 같이 깨진다.
- 모달 폭 `min(1080px,96vw)` → **`min(1480px,97vw)`**, `#chDrillBody` 패딩 18px → 10/12/16px.
- ⚠ **`colgroup`이 필수다.** `.chwape`는 `table-layout:fixed`라 col이 없으면 열이 균등 분할돼
  품목명이 잘리고 숫자 열엔 여백이 남는다. 품목(300px)·품목코드(104px)·숫자열 고정, 조치의견만 auto.
...
## 소진계획 메모 키 — 행(배치) 단위 (2026-08-19) · ①탭 드릴다운 + 📡 Sensing 공통
...
- ⚠ **폴백은 새 키에 기록이 "아예 없을 때"만.** 빈 문자열로 지운 것도 기록이다(`find`는 없을 때만 undefined) —
  안 그러면 지운 메모가 되살아난다
- **📡 Aging Sensing 표(`_renderSenTbody`)도 같은 규칙이다** — 키는 `SENSING|<M+N>|<구간>|<채널>|<자재>|<배치>|<플랜트>|<저장위치>`.
  예전 키로는 26/8 '18개월 이상' M+0이 259행에 고유키 72개였다. 붙이면 3구간×3개월 1,093행 중복 0
...
## 월별 미소진 Risk 이력 (2026-08-19 신설)

SKU 상세 팝업의 「월별 Risk 추적」 표. **발행할 때마다 그 달의 risk를 얼려서 쌓는다.**

### 저장 규칙 — "기준월이 지나면 얼어붙는다"
| | |
|---|---|
| 정체성 | **기준월(`base_yearmonth`)** 하나. 파일 버전(v1.1/v1.4)이 아니다 |
| 당월 | **매번 갱신** (아직 확정 전) |
| 과거월 | **동결.** KV/리포에 이미 있으면 파일이 있어도 다시 계산하지 않는다 |
| 단가 | **값으로 박아 저장**(`rows[].unit`) → 결산 표준원가 파일이 갱신돼도 과거월 금액은 안 움직인다 |
| 되돌리기 | `npm run deploy -- --rebuild-risk` (로직 버그를 고쳤을 때만) |

### ⚠ 3중 저장 — 하나라도 빼면 이력이 날아간다
1. **`data/risk_history.json` — 리포에 커밋되는 원본.** git 히스토리가 곧 세대별 백업이다.
   xlsx는 `.gitignore`라 Codespace가 새로 만들어지면 사라지지만 이 파일은 남는다. **반드시 커밋할 것.**
2. **KV `dashboard:live`의 `risk_history`** — 화면이 읽는 사본
3. **가드 3개** (`tools/deploy.mjs`)
   - KV 읽기 실패 → **발행 중단**. ⚠ `loadSharedDashboardData()`는 **네트워크 실패도 `false`로 삼키므로**
     그것만 보면 '데이터 없음'과 구분이 안 되어 빈 이력으로 덮는다. `readLiveRiskHistory`가
     **`?meta=1`을 먼저 직접 fetch**해서 도달 가능성과 `has`를 확인한 뒤에야 본문을 읽는다
   - 이력 개월 수가 줄면 **중단** (`--force-shrink` 없이는)
   - 병합은 **합집합**(`mergeRiskHistories`) — 같은 달은 `builtAt`이 최신인 쪽. 리포에만 있는 달도 살아남는다
   - ⚠ 같은 구멍이 **`readLivePkgSnapshot`에는 아직 남아 있다** (실패 시 조용히 null → 전월대비 카드 소실)

### 구현 통로
- `buildRiskSnapshot(result)` / `mergeRiskHistory(carried,snap,rebuild)` — 대시보드 스크립트 안.
  배포 도구와 브라우저가 **같은 코드**를 쓴다
- ⚠ **`UPLOADED`를 확정한 뒤에** 계산해야 한다 — `computeSoojin`·`getPsiPriceMap`이 `UPLOADED`를 읽는다.
  그래서 `handleUpload`이 `UPLOADED`를 두 번 세팅한다(계산용 → 이력을 붙인 최종본)
- ⚠ **이어받을 이력은 `UPLOADED`가 덮이기 전에** 잡아 둔다(`carriedRiskHistory`). 뒤에 읽으면
  방금 만든 값(=이력 없음)을 이어받아 **과거 이력이 통째로 사라진다**
- `INVENTORY_SHEET_INPUT` — 과거월 risk를 뽑을 때 어느 원장 시트를 볼지 지정. 26/7 파일은
  기초재고=6/1 / 현재고=7/8 두 달을 담고 있어, 안 지정하면 ①탭 월 필터(7월)와 다른 달이 잡힌다
- 크기: 한 달 약 25KB(115~127행). `dashboard:live` 상한 20MB라 여유가 크다

### 화면 규칙
- ⚠ **계획 열을 빼지 말 것.** 26/8 `7301807`이 risk 26,641 → 14,958로 반토막 났는데 실제 소진은 916개뿐이고
  나머지는 월평균 계획이 1,039 → 3,384로 오른 효과였다. 계획을 안 보여주면 **표가 사람을 속인다.**
  표 아래 해석 문장(`renderRiskTrend`의 `note`)이 이 분해를 자동으로 써 준다 — 이게 이 기능의 핵심 가치다
- **실적은 얼리지 않고 당월 파일의 `hit.qty.byMonth`에서 live로 읽는다** — 과거월 파일의 실적은 미확정본이라
  당월 파일 쪽이 항상 더 최신이다
- ⚠ `riskMonthActualQty`는 **채널로 거르지 않는다.** 원장 채널(사업부·유통구분)과 오차율 시트 채널(판매채널)은
  서로 다른 축이라 교집합이 없다 — 필터를 걸면 전부 0이 된다

## 소진 판단(`computeSoojin`) — 2026-08-19 수정 두 가지

1. **기준 시점이 파일을 따라간다.** 예전엔 `new Date(2026,5,1)`(6/1)이 박혀 있어 26/7·26/8 파일이
   똑같이 마감일 `26/11/2`를 냈다. 지금은 `soojinBaseDate()`(= `base_yearmonth` 1일)를 쓴다
   (26/8 → `27/1/2`). 월별 일수 배분도 기준월부터 센다. **금액·수량은 `validDays`만 쓰므로 영향이 거의 없고
   (일수 배분 차이로 26/8 7301807이 14,954 → 14,958), 틀렸던 건 표시용 날짜였다.**
   업로드 데이터가 없는 데모 상태는 옛 값(6/1)으로 폴백한다
2. **원가 환산이 `getPsiPriceMap()`으로 바뀌었다.** 예전엔 원가시트 배율 역산이라 같은 품목이
   PSI 탭 2,900원 / Aging risk 2,093원으로 갈렸다. 26/8 기준 **총액은 355.4 → 363.9백만(2%)** 이지만
   **개별 품목은 크게 움직인다**(7301807 31.3 → 43.4백만). `risk금액 > 재고금액` 붕괴는 1건 → 0건.
   ⚠ **원장 금액÷수량을 1순위로 올리지 말 것** — 26/8 수량 기준 35.2%가 금액 0이라
   멀티비타민 미네랄 2원(표준원가 2,863원)처럼 무너진다. PSI에 없는 자재는 옛 배율 역산으로 폴백한다

## 업무 규칙 요약 (자세한 건 memory 참고)
- 26/9부터 `기준정보`에 `계층구분`(S급/A급/B급/C급/N)이 있다. `parseHierarchyMap` →
  `applyHierarchyToResult`로 Aging·PSI·오차분석에 공통 적용한다. 화면마다 별도 분류 로직을 만들지 말 것.
- 26/9 `다이소(PSI)`는 전체 마스터가 아니라 9개 통합코드만 담는다. 마스터에만 있는 코드는 일반 PSI 원본으로
  유지하고, PSI에만 있는 코드/품목명 불일치만 업로드 오류로 본다.
