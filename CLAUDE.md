...
- ⚠ **두 스냅샷 간격이 항상 1개월인 건 아니다.** 26/7 파일의 기초재고는 6/1 기준이라 26/8(8/1)과 두 달 벌어진다.
  `renderPkgDelta`가 `yearmonth` 차이를 보고 머리말을 `전월`/`N개월 전`으로 바꾼다(숫자·계산은 그대로).
  정확히 한 달 전을 보려면 그 달 기초재고가 든 파일을 `--prev=`로 직접 지정할 것.
