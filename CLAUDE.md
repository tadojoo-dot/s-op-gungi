...
| | |
|---|---|
| 적중률 열 | 오차율 그룹 **바로 오른쪽**에 같은 월 수·**같은 폭(`W_MO`)·같은 타이포**로 붙인다(2026-08-19 사용자 지정: "동일한 열로"). 위계를 주지 않는다. 그룹 배경도 같은 톤이고 경계는 `cw-sep` 세로선뿐. |
| 채널별 오차기여도 | **최근 표시 오차율 월 1개**를 기준으로 `채널 |차이| ÷ 전체 |차이|`를 계산한다. 적중률 뒤, 실적 구간 바로 앞에 두며 전체 합계는 100%다. |
| 월 열 | `CHWAPE_MONTHS_SHOWN=2`(오차율) / `CHWAPE_ACTUAL_MONTHS_SHOWN=4`(실적, **최근**) / `CHWAPE_PLAN_MONTHS_SHOWN=3`(계획, **향후**). 최근 WAPE 월은 `당초계획·실적·차이` 비교 3열로 빼고, 같은 달은 SF 실적 이력에서 중복 제거한다. `buildChannelWape`·`__summary.ch`는 전 구간을 계속 계산한다(정렬 키·셀 클릭 팝업이 과거월을 쓴다) — 상수만 바꾸면 늘어난다 |
| 정렬 | 표 데이터는 가운데 정렬. 입력 중인 조치의견 텍스트만 왼쪽 정렬 |
| 실적·계획 열 | `channelPlanActual` / `channelMonthCells` — 판매계획(SF) `__summary.ch`의 `actual[]`/`plan[]`을 **월별 숫자**로. 최근 WAPE 월의 비교 3열은 `buildChannelWape().rows[].cells`의 `plan`·`actual`을 사용하고, `차이=actual-plan`의 부호 있는 순차이다. 계획 구간은 옅은 배경(`.cw-plan`). ⚠ `channelPlanActual`이 표시 개월만 잘라서 돌려주는데, **월 배열과 값 배열을 같은 인덱스로 함께** 잘라야 한다 — 값이 위치 기반이라 한쪽만 자르면 조용히 다른 달 숫자가 박힌다 |
| 단위 | ⚠ **금액(억원) 고정.** `__summary.ch`에 수량이 없어 위 금액/수량 토글과 무관하다 |
| 헤더 | ⚠ **2단 헤더**다 — 월 라벨이 오차율·적중률·실적·계획 구간에 겹쳐 나오므로 위에 그룹을 둔다. 오차기여도는 2행을 합친 단일 열이다. |
| 표 폭 | 숫자 열은 컴팩트하게 px 고정하고 조치의견 열은 `<col>`에 width를 주지 않는다(auto). `width:100%`로 1280px 기준 화면 안에 들어오도록 한다. |
| 타이포 위계 | 오차율 13px/800 · 월별 실적/계획(`.cw-mv`) 10.5px/600 · 오차기여도 11px/800. 표 전용 축소 규칙은 `.chwape-lg`에만 둔다 |
...
| ⚠ 소스 차이 | 오차율(%)과 최근 WAPE 월의 `당초계획·실적·차이` 비교는 `오차율 분석` 시트, 과거 `실적 이력`과 향후 `판매계획`은 `판매계획(SF)` 시트다. 두 시트의 집계 기준이 다를 수 있으므로 그룹 머리글과 하단 설명에서 원천을 구분한다. **하나로 맞추려 들지 말 것** |
...
- 키에 **Aging 구간(`cat`)이 들어 있는 것은 유지**한다(새 행별 기록이 서로 섞이면 안 됨).
  다만 2026-09-14부터 옛 형식 기록(`AGING|채널|구간|자재`)은 정확한 구간이 없거나 납품기한 보기로 전환된 경우에도
  `채널+자재` 기준으로 같은 자재의 금액 최대 행에 **표시만** 이어받는다. 새 키로 자동 변환하거나 저장하지 않는다.
