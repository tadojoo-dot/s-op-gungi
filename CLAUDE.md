...
- ⚠ **파일 하나가 여러 달을 담는다.** 26/7 파일은 `기초재고`=6/1, `현재고`=7/8이다 →
  파일 2개로 **6·7·8월 3개월**이 나온다. `findPrevAgingExcels`(복수) + `readPrevMonths`가
  과거 파일의 모든 원장 시트를 훑어 **달마다 하나씩** 뽑는다(같은 달이면 `기초재고` 우선 — 당월이 월초 기준이라).
  26/8 기준: 6월 62.20(기초재고 6/1) / 7월 62.23(현재고 7/8) / 8월 78.66(기초재고 8/1, 당월)
...
- KV 영향: 두 달치 매트릭스 추가에 gzip +94KB(282→376KB). 상한 20MB라 여유 있다.
...
배포 로그에도 `추이 7월: 현황 52.40억 → 원장 62.23억으로 보정`처럼 찍힌다.
  ...
  26/8 배포 기준: 7월 52.40→**62.23** / 6월 61.00→**62.20** / 8월 변화 없음(78.66)
