...
| 스파크라인 | `channelPlanActual` / `channelSparkCell` — 판매계획(SF) `__summary.ch`의 `actual[1~7]`/`plan[8~12]`. **인라인 SVG**로 그린다 (⚠ Chart.js 금지: 행마다 인스턴스가 생겨 무겁고 인쇄가 깨진다) |
| 스파크라인 단위 | ⚠ **금액(억원) 고정.** `__summary.ch`에 수량이 없어 위 금액/수량 토글과 무관하다 |
| 스케일 | 행마다 따로 잡는다 — 채널 간 크기 비교가 아니라 **채널 내 추세**를 보는 열이다 |
