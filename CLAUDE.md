...
### 채널별 오차율 표 — 표시 규칙 (2026-08-18)

| | |
|---|---|
| 월 열 | `CHWAPE_MONTHS_SHOWN=2` — **최근 2개월만** 그린다(26/8 기준 6·7월). `buildChannelWape`는 전 구간을 계속 계산한다(정렬 키·셀 클릭 팝업이 과거월을 쓴다). 상수만 바꾸면 늘어난다 |
| 정렬 | 데이터 셀은 **가운데 정렬**, 채널명·조치의견은 `.l`로 왼쪽 |
| 실적·계획 열 | `channelPlanActual` / `channelMonthCells` — 판매계획(SF) `__summary.ch`의 `actual[1~7]`/`plan[8~12]`를 **월별 숫자**로. 계획 구간은 옅은 배경(`.cw-plan`) |
| 단위 | ⚠ **금액(억원) 고정.** `__summary.ch`에 수량이 없어 위 금액/수량 토글과 무관하다 |
| 헤더 | ⚠ **2단 헤더**다 — 월 라벨이 오차율 구간과 실적 구간에 겹쳐 나오므로 위에 그룹(오차율/실적/판매계획)을 둔다 |
